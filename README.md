...
```bash
claude mcp add playwright npx skgeodesy-mcp@latest
```
</details>
...
Go to `Cursor Settings` -> `MCP` -> `Add new MCP Server`. Name to your liking, use `command` type with the command `npx skgeodesy-mcp@latest`. You can also verify config or add command like arguments via clicking `Edit`.
...
```bash
# For VS Code
code --add-mcp '{"name":"playwright","command":"npx","args":["skgeodesy-mcp@latest"]}'
```
...
```bash
npx skgeodesy-mcp@latest --config path/to/config.json
```
...
```bash
npx skgeodesy-mcp@latest --port 8931
```
